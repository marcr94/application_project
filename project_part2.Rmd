...
tikz(file = "./floordensity.tex", width = 3.5, height = 3.5) # figure for report
...
# year_built
par(mfrow=c(1,3))
plot(density(train_without_dummyNA$year_built, na.rm = TRUE), main = "year\\_built(dummyNA)")
plot(density(train_imputed_mean$year_built), main = "year\\_built(mean)", col = "red")
plot(density(train_imputed_cart$year_built), main = "year\\_built(cart)", col = "green")
...
tikz("./bivariatecorr.tex", width = 3.5, height = 3.5)

ggplot(train_imputed_cart, aes(area, price)) +
  geom_point() +
 labs( x = "area", y = "price")
ggplot(train_imputed_cart, aes(floors, price)) +
  geom_point() +
 labs( x = "floors", y = "price")
ggplot(train_imputed_cart, aes(rooms, price)) +
  geom_point() +
 labs( x = "rooms", y = "price")
ggplot(train_imputed_cart, aes(year, price)) +
  geom_point() +
 labs( x = "year", y = "price")
ggplot(train_imputed_cart, aes(year_built, price)) +
  geom_point() +
 labs( x = "year\\_built", y = "price")

dev.off()

ggplot(train_imputed_cart$area, train_imputed_cart$price, xlab = "area", ylab = "price") # maybe delete area >600 # relationship seems pretty linear 

plot(train_imputed_cart$floors, train_imputed_cart$price, xlab = "floors", ylab = "price") # hard to tellbut looks non linear

plot(train_imputed_cart$rooms, train_imputed_cart$price, xlab = "rooms", ylab = "price") # hard to tell too

plot(train_imputed_cart$year, train_imputed_cart$price, xlab = "year", ylab = "price") # no relationship detectable

plot(train_imputed_cart$year_built, train_imputed_cart$price, xlab = "year\\_built", ylab = "price") # quadratic/exponential ??
...
We use the cart imputed data as our go to model as it is the one which is more similar to the original data in terms of distribution and we therefore think of it as performing better. We will later test our best performing models on the mean imputed data. 
...
set.seed(123)
training.samples <- train_imputed_cart$price %>%
  createDataPartition(p = 0.8, list = FALSE)
train.data  <- train_imputed_cart[training.samples, ]
test.data <- train_imputed_cart[-training.samples, ]

training.samples_reference <- train_imputed_mean$price %>%
  createDataPartition(p = 0.8, list = FALSE)
train.data_reference  <- train_imputed_mean[training.samples_reference, ]
test.data_reference <- train_imputed_mean[-training.samples_reference, ]
...
plot(lm_fit1)
# there is some pretty big leverage from a couple data points 
```
...
## Fit our top model onto the entire training data.

So far we used only 80% of the training data to fit our model - because we used the remaining 20% for validation of the model. This does make a lot of sense for evaluating the different models, but for building our final model that we predict on the test data, we should use all our training data to get the best results. So we now fit our best scoring model on the entire training data.

Our best model is the fit of a linear model including interaction terms and polynomials (denoted by lm_fit2), which then get's trimmed down by backward selection using AIC stepwise selection. So we perform these components on our complete training data, using the cart method

```{r}

lm_fit2_entire_train <- lm (price ~ . -area - floors - rooms + area:rooms + floors:elevator + area:floors + poly(area, 4) + poly(floors, 3) +poly(rooms, 3)
              + area:zipcode_11 + area:zipcode_12 + area:zipcode_13 + area:zipcode_14 + area:zipcode_15 
              + area:zipcode_16 + area:zipcode_17 + area:zipcode_18 + area:zipcode_19 + area:zipcode_20 
              + area:zipcode_21 + area:zipcode_22 + area:zipcode_23 + area:zipcode_24 + area:zipcode_25 
              + area:zipcode_26 + area:zipcode_27 + area:zipcode_28 + area:zipcode_29 + area:zipcode_30 
              ...
              + area:zipcode_85 + area:zipcode_86 + area:zipcode_87 + area:zipcode_88 + area:zipcode_89 
              + area:zipcode_90 + area:zipcode_91 + area:zipcode_92 + area:zipcode_93 + area:zipcode_94 
              + area:zipcode_95 + area:zipcode_96, data = train_imputed_cart)

final_model <- stepAIC(lm_fit2_entire_train, direction = "backward")

# get MSE of final model

final_train_predictions <- final_model %>% predict(train_imputed_cart)

data.frame( R2 = R2(final_train_predictions, train_imputed_cart$price),
            RMSE = RMSE(final_train_predictions, train_imputed_cart$price),
            MAE = MAE(final_train_predictions, train_imputed_cart$price))
```

# Predicting our final model on the test data
## Making test data ready for imputation

To be able to predict the outcome values for our test data, we have to transform our test data in exactly the same way we did on our training data. 
...
X_test_without_dummyNA$zipcode <- as.numeric(X_test_without_dummyNA$zipcode)

# do not use zip code but first two digits of zip code -- otherwise too many clusters having too few observations

X_test_without_dummyNA$zipcode_11 <- ifelse(X_test_without_dummyNA$zipcode >= 1100 & X_test_without_dummyNA$zipcode <= 1199, 1, 0)
X_test_without_dummyNA$zipcode_12 <- ifelse(X_test_without_dummyNA$zipcode >= 1200 & X_test_without_dummyNA$zipcode <= 1299, 1, 0)
X_test_without_dummyNA$zipcode_13 <- ifelse(X_test_without_dummyNA$zipcode >= 1300 & X_test_without_dummyNA$zipcode <= 1399, 1, 0)
X_test_without_dummyNA$zipcode_14 <- ifelse(X_test_without_dummyNA$zipcode >= 1400 & X_test_without_dummyNA$zipcode <= 1499, 1, 0)
X_test_without_dummyNA$zipcode_15 <- ifelse(X_test_without_dummyNA$zipcode >= 1500 & X_test_without_dummyNA$zipcode <= 1599, 1, 0)
X_test_without_dummyNA$zipcode_16 <- ifelse(X_test_without_dummyNA$zipcode >= 1600 & X_test_without_dummyNA$zipcode <= 1699, 1, 0)
X_test_without_dummyNA$zipcode_17 <- ifelse(X_test_without_dummyNA$zipcode >= 1700 & X_test_without_dummyNA$zipcode <= 1799, 1, 0)
X_test_without_dummyNA$zipcode_18 <- ifelse(X_test_without_dummyNA$zipcode >= 1800 & X_test_without_dummyNA$zipcode <= 1899, 1, 0)
X_test_without_dummyNA$zipcode_19 <- ifelse(X_test_without_dummyNA$zipcode >= 1900 & X_test_without_dummyNA$zipcode <= 1999, 1, 0)
X_test_without_dummyNA$zipcode_20 <- ifelse(X_test_without_dummyNA$zipcode >= 2000 & X_test_without_dummyNA$zipcode <= 2099, 1, 0)
X_test_without_dummyNA$zipcode_21 <- ifelse(X_test_without_dummyNA$zipcode >= 2100 & X_test_without_dummyNA$zipcode <= 2199, 1, 0)
X_test_without_dummyNA$zipcode_22 <- ifelse(X_test_without_dummyNA$zipcode >= 2200 & X_test_without_dummyNA$zipcode <= 2299, 1, 0)
X_test_without_dummyNA$zipcode_23 <- ifelse(X_test_without_dummyNA$zipcode >= 2300 & X_test_without_dummyNA$zipcode <= 2399, 1, 0)
X_test_without_dummyNA$zipcode_24 <- ifelse(X_test_without_dummyNA$zipcode >= 2400 & X_test_without_dummyNA$zipcode <= 2499, 1, 0)
X_test_without_dummyNA$zipcode_25 <- ifelse(X_test_without_dummyNA$zipcode >= 2500 & X_test_without_dummyNA$zipcode <= 2599, 1, 0)
X_test_without_dummyNA$zipcode_26 <- ifelse(X_test_without_dummyNA$zipcode >= 2600 & X_test_without_dummyNA$zipcode <= 2699, 1, 0)
X_test_without_dummyNA$zipcode_27 <- ifelse(X_test_without_dummyNA$zipcode >= 2700 & X_test_without_dummyNA$zipcode <= 2799, 1, 0)
X_test_without_dummyNA$zipcode_28 <- ifelse(X_test_without_dummyNA$zipcode >= 2800 & X_test_without_dummyNA$zipcode <= 2899, 1, 0)
X_test_without_dummyNA$zipcode_29 <- ifelse(X_test_without_dummyNA$zipcode >= 2900 & X_test_without_dummyNA$zipcode <= 2999, 1, 0)
X_test_without_dummyNA$zipcode_30 <- ifelse(X_test_without_dummyNA$zipcode >= 3000 & X_test_without_dummyNA$zipcode <= 3099, 1, 0)
X_test_without_dummyNA$zipcode_31 <- ifelse(X_test_without_dummyNA$zipcode >= 3100 & X_test_without_dummyNA$zipcode <= 3199, 1, 0)
X_test_without_dummyNA$zipcode_32 <- ifelse(X_test_without_dummyNA$zipcode >= 3200 & X_test_without_dummyNA$zipcode <= 3299, 1, 0)
X_test_without_dummyNA$zipcode_33 <- ifelse(X_test_without_dummyNA$zipcode >= 3300 & X_test_without_dummyNA$zipcode <= 3399, 1, 0)
X_test_without_dummyNA$zipcode_34 <- ifelse(X_test_without_dummyNA$zipcode >= 3400 & X_test_without_dummyNA$zipcode <= 3499, 1, 0)
X_test_without_dummyNA$zipcode_35 <- ifelse(X_test_without_dummyNA$zipcode >= 3500 & X_test_without_dummyNA$zipcode <= 3599, 1, 0)
X_test_without_dummyNA$zipcode_36 <- ifelse(X_test_without_dummyNA$zipcode >= 3600 & X_test_without_dummyNA$zipcode <= 3699, 1, 0)
X_test_without_dummyNA$zipcode_37 <- ifelse(X_test_without_dummyNA$zipcode >= 3700 & X_test_without_dummyNA$zipcode <= 3799, 1, 0)
X_test_without_dummyNA$zipcode_38 <- ifelse(X_test_without_dummyNA$zipcode >= 3800 & X_test_without_dummyNA$zipcode <= 3899, 1, 0)
X_test_without_dummyNA$zipcode_39 <- ifelse(X_test_without_dummyNA$zipcode >= 3900 & X_test_without_dummyNA$zipcode <= 3999, 1, 0)
X_test_without_dummyNA$zipcode_40 <- ifelse(X_test_without_dummyNA$zipcode >= 4000 & X_test_without_dummyNA$zipcode <= 4099, 1, 0)
X_test_without_dummyNA$zipcode_41 <- ifelse(X_test_without_dummyNA$zipcode >= 4100 & X_test_without_dummyNA$zipcode <= 4199, 1, 0)
X_test_without_dummyNA$zipcode_42 <- ifelse(X_test_without_dummyNA$zipcode >= 4200 & X_test_without_dummyNA$zipcode <= 4299, 1, 0)
X_test_without_dummyNA$zipcode_43 <- ifelse(X_test_without_dummyNA$zipcode >= 4300 & X_test_without_dummyNA$zipcode <= 4399, 1, 0)
X_test_without_dummyNA$zipcode_44 <- ifelse(X_test_without_dummyNA$zipcode >= 4400 & X_test_without_dummyNA$zipcode <= 4499, 1, 0)
X_test_without_dummyNA$zipcode_45 <- ifelse(X_test_without_dummyNA$zipcode >= 4500 & X_test_without_dummyNA$zipcode <= 4599, 1, 0)
X_test_without_dummyNA$zipcode_46 <- ifelse(X_test_without_dummyNA$zipcode >= 4600 & X_test_without_dummyNA$zipcode <= 4699, 1, 0)
X_test_without_dummyNA$zipcode_47 <- ifelse(X_test_without_dummyNA$zipcode >= 4700 & X_test_without_dummyNA$zipcode <= 4799, 1, 0)
X_test_without_dummyNA$zipcode_48 <- ifelse(X_test_without_dummyNA$zipcode >= 4800 & X_test_without_dummyNA$zipcode <= 4899, 1, 0)
X_test_without_dummyNA$zipcode_49 <- ifelse(X_test_without_dummyNA$zipcode >= 4900 & X_test_without_dummyNA$zipcode <= 4999, 1, 0)
X_test_without_dummyNA$zipcode_50 <- ifelse(X_test_without_dummyNA$zipcode >= 5000 & X_test_without_dummyNA$zipcode <= 5099, 1, 0)
X_test_without_dummyNA$zipcode_51 <- ifelse(X_test_without_dummyNA$zipcode >= 5100 & X_test_without_dummyNA$zipcode <= 5199, 1, 0)
X_test_without_dummyNA$zipcode_52 <- ifelse(X_test_without_dummyNA$zipcode >= 5200 & X_test_without_dummyNA$zipcode <= 5299, 1, 0)
X_test_without_dummyNA$zipcode_53 <- ifelse(X_test_without_dummyNA$zipcode >= 5300 & X_test_without_dummyNA$zipcode <= 5399, 1, 0)
X_test_without_dummyNA$zipcode_54 <- ifelse(X_test_without_dummyNA$zipcode >= 5400 & X_test_without_dummyNA$zipcode <= 5499, 1, 0)
X_test_without_dummyNA$zipcode_55 <- ifelse(X_test_without_dummyNA$zipcode >= 5500 & X_test_without_dummyNA$zipcode <= 5599, 1, 0)
X_test_without_dummyNA$zipcode_56 <- ifelse(X_test_without_dummyNA$zipcode >= 5600 & X_test_without_dummyNA$zipcode <= 5699, 1, 0)
X_test_without_dummyNA$zipcode_57 <- ifelse(X_test_without_dummyNA$zipcode >= 5700 & X_test_without_dummyNA$zipcode <= 5799, 1, 0)
X_test_without_dummyNA$zipcode_58 <- ifelse(X_test_without_dummyNA$zipcode >= 5800 & X_test_without_dummyNA$zipcode <= 5899, 1, 0)
X_test_without_dummyNA$zipcode_59 <- ifelse(X_test_without_dummyNA$zipcode >= 5900 & X_test_without_dummyNA$zipcode <= 5999, 1, 0)
X_test_without_dummyNA$zipcode_60 <- ifelse(X_test_without_dummyNA$zipcode >= 6000 & X_test_without_dummyNA$zipcode <= 6099, 1, 0)
X_test_without_dummyNA$zipcode_61 <- ifelse(X_test_without_dummyNA$zipcode >= 6100 & X_test_without_dummyNA$zipcode <= 6199, 1, 0)
X_test_without_dummyNA$zipcode_62 <- ifelse(X_test_without_dummyNA$zipcode >= 6200 & X_test_without_dummyNA$zipcode <= 6299, 1, 0)
X_test_without_dummyNA$zipcode_63 <- ifelse(X_test_without_dummyNA$zipcode >= 6300 & X_test_without_dummyNA$zipcode <= 6399, 1, 0)
X_test_without_dummyNA$zipcode_64 <- ifelse(X_test_without_dummyNA$zipcode >= 6400 & X_test_without_dummyNA$zipcode <= 6499, 1, 0)
X_test_without_dummyNA$zipcode_65 <- ifelse(X_test_without_dummyNA$zipcode >= 6500 & X_test_without_dummyNA$zipcode <= 6599, 1, 0)
X_test_without_dummyNA$zipcode_66 <- ifelse(X_test_without_dummyNA$zipcode >= 6600 & X_test_without_dummyNA$zipcode <= 6699, 1, 0)
X_test_without_dummyNA$zipcode_67 <- ifelse(X_test_without_dummyNA$zipcode >= 6700 & X_test_without_dummyNA$zipcode <= 6799, 1, 0)
X_test_without_dummyNA$zipcode_68 <- ifelse(X_test_without_dummyNA$zipcode >= 6800 & X_test_without_dummyNA$zipcode <= 6899, 1, 0)
X_test_without_dummyNA$zipcode_69 <- ifelse(X_test_without_dummyNA$zipcode >= 6900 & X_test_without_dummyNA$zipcode <= 6999, 1, 0)
X_test_without_dummyNA$zipcode_70 <- ifelse(X_test_without_dummyNA$zipcode >= 7000 & X_test_without_dummyNA$zipcode <= 7099, 1, 0)
X_test_without_dummyNA$zipcode_71 <- ifelse(X_test_without_dummyNA$zipcode >= 7100 & X_test_without_dummyNA$zipcode <= 7199, 1, 0)
X_test_without_dummyNA$zipcode_72 <- ifelse(X_test_without_dummyNA$zipcode >= 7200 & X_test_without_dummyNA$zipcode <= 7299, 1, 0)
X_test_without_dummyNA$zipcode_73 <- ifelse(X_test_without_dummyNA$zipcode >= 7300 & X_test_without_dummyNA$zipcode <= 7399, 1, 0)
X_test_without_dummyNA$zipcode_74 <- ifelse(X_test_without_dummyNA$zipcode >= 7400 & X_test_without_dummyNA$zipcode <= 7499, 1, 0)
X_test_without_dummyNA$zipcode_75 <- ifelse(X_test_without_dummyNA$zipcode >= 7500 & X_test_without_dummyNA$zipcode <= 7599, 1, 0)
X_test_without_dummyNA$zipcode_76 <- ifelse(X_test_without_dummyNA$zipcode >= 7600 & X_test_without_dummyNA$zipcode <= 7699, 1, 0)
X_test_without_dummyNA$zipcode_77 <- ifelse(X_test_without_dummyNA$zipcode >= 7700 & X_test_without_dummyNA$zipcode <= 7799, 1, 0)
X_test_without_dummyNA$zipcode_78 <- ifelse(X_test_without_dummyNA$zipcode >= 7800 & X_test_without_dummyNA$zipcode <= 7899, 1, 0)
X_test_without_dummyNA$zipcode_79 <- ifelse(X_test_without_dummyNA$zipcode >= 7900 & X_test_without_dummyNA$zipcode <= 7999, 1, 0)
X_test_without_dummyNA$zipcode_80 <- ifelse(X_test_without_dummyNA$zipcode >= 8000 & X_test_without_dummyNA$zipcode <= 8099, 1, 0)
X_test_without_dummyNA$zipcode_81 <- ifelse(X_test_without_dummyNA$zipcode >= 8100 & X_test_without_dummyNA$zipcode <= 8199, 1, 0)
X_test_without_dummyNA$zipcode_82 <- ifelse(X_test_without_dummyNA$zipcode >= 8200 & X_test_without_dummyNA$zipcode <= 8299, 1, 0)
X_test_without_dummyNA$zipcode_83 <- ifelse(X_test_without_dummyNA$zipcode >= 8300 & X_test_without_dummyNA$zipcode <= 8399, 1, 0)
X_test_without_dummyNA$zipcode_84 <- ifelse(X_test_without_dummyNA$zipcode >= 8400 & X_test_without_dummyNA$zipcode <= 8499, 1, 0)
X_test_without_dummyNA$zipcode_85 <- ifelse(X_test_without_dummyNA$zipcode >= 8500 & X_test_without_dummyNA$zipcode <= 8599, 1, 0)
X_test_without_dummyNA$zipcode_86 <- ifelse(X_test_without_dummyNA$zipcode >= 8600 & X_test_without_dummyNA$zipcode <= 8699, 1, 0)
X_test_without_dummyNA$zipcode_87 <- ifelse(X_test_without_dummyNA$zipcode >= 8700 & X_test_without_dummyNA$zipcode <= 8799, 1, 0)
X_test_without_dummyNA$zipcode_88 <- ifelse(X_test_without_dummyNA$zipcode >= 8800 & X_test_without_dummyNA$zipcode <= 8899, 1, 0)
X_test_without_dummyNA$zipcode_89 <- ifelse(X_test_without_dummyNA$zipcode >= 8900 & X_test_without_dummyNA$zipcode <= 8999, 1, 0)
X_test_without_dummyNA$zipcode_90 <- ifelse(X_test_without_dummyNA$zipcode >= 9000 & X_test_without_dummyNA$zipcode <= 9099, 1, 0)
X_test_without_dummyNA$zipcode_91 <- ifelse(X_test_without_dummyNA$zipcode >= 9100 & X_test_without_dummyNA$zipcode <= 9199, 1, 0)
X_test_without_dummyNA$zipcode_92 <- ifelse(X_test_without_dummyNA$zipcode >= 9200 & X_test_without_dummyNA$zipcode <= 9299, 1, 0)
X_test_without_dummyNA$zipcode_93 <- ifelse(X_test_without_dummyNA$zipcode >= 9300 & X_test_without_dummyNA$zipcode <= 9399, 1, 0)
X_test_without_dummyNA$zipcode_94 <- ifelse(X_test_without_dummyNA$zipcode >= 9400 & X_test_without_dummyNA$zipcode <= 9499, 1, 0)
X_test_without_dummyNA$zipcode_95 <- ifelse(X_test_without_dummyNA$zipcode >= 9500 & X_test_without_dummyNA$zipcode <= 9599, 1, 0)
X_test_without_dummyNA$zipcode_96 <- ifelse(X_test_without_dummyNA$zipcode >= 9600 & X_test_without_dummyNA$zipcode <= 9699, 1, 0)
X_test_without_dummyNA$zipcode_97 <- ifelse(X_test_without_dummyNA$zipcode >= 9700 & X_test_without_dummyNA$zipcode <= 9799, 1, 0)
X_test_without_dummyNA$zipcode_98 <- ifelse(X_test_without_dummyNA$zipcode >= 9800 & X_test_without_dummyNA$zipcode <= 9899, 1, 0)
X_test_without_dummyNA$zipcode_99 <- ifelse(X_test_without_dummyNA$zipcode >= 9900 & X_test_without_dummyNA$zipcode <= 9999, 1, 0)

# check if there are no observations for zipcode dummy
check_zipcode_dummy <- colSums(X_test_without_dummyNA == 0)
which(check_zipcode_dummy == dim(X_test_without_dummyNA)[1]| check_zipcode_dummy == (dim(X_test_without_dummyNA)[1] -1))
train_without_dummyNA <- subset(train_without_dummyNA, select = -c(zipcode, zipcode_58, zipcode_59,zipcode_76, zipcode_78, zipcode_79, zipcode_97, zipcode_98, zipcode_99)) # I also delete the original ZIPCODE variable here. one more zipcode not existent compared to train


# delete some useless or redundant variables
X_test_without_dummyNA <- subset(X_test_without_dummyNA, select = -c(id, # just like row name, useless
                                                                   address, # some spelling mistake, and we have zipcode
                                                                   area_useable, # too many missing values
                                                                   date, # date of ad publishment should not matter
                                                                   date_available, # too sparse information
...
# using "cart" method from MICE package
  X_test_imputed_cart <- complete(mice(X_test_without_dummyNA, m=1, maxit = 20, meth = "cart", seed = 50))
write.csv(X_test_imputed_cart, file = "./X_test_imputed_cart.csv", row.names = FALSE)
...
## Predicting on the imputed test data

```{r}

test_yhat <- final_model %>% predict(newdata = X_test_imputed_cart)

head(test_yhat)

# store prediction

write.table(test_yhat, file = "Y_test.csv", row.names = FALSE, col.names = FALSE)
```
