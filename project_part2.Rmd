...
train_imputed <- sapply(train_imputed, function(x)impute(x, fun = median))
...
# What if we create a new variable, "age"? We find that some are built in the future, 2021 or 2022...
train_without_dummyNA$age <- 2020 - train_without_dummyNA$year_built

# delete some useless or redundant variables
train_imputation <- subset(train_without_dummyNA, select = -c(id, address, area_useable, date, date_available, municipality, new_building, newly_built, oldbuilding, price, street, year, year_built))
...
train_imputation_pmm <- complete(mice(train_imputation, m=1, maxit = 1, meth = "pmm", seed = 50), 1) 
#write.csv(train_imputation_pmm, file = "C:/Users/Young/Desktop/Big Data_Project/20_Part2_project/Part2_project/train_imputation_pmm.csv", row.names = FALSE)

# using "rf" method from MICE package
train_imputation_rf <- complete(mice(train_imputation, m=1, maxit = 1, meth = "rf", seed = 50), 1) 
#write.csv(train_imputation_rf, file = "C:/Users/Young/Desktop/Big Data_Project/20_Part2_project/Part2_project/train_imputation_rf.csv", row.names = FALSE)
