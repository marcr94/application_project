...
train_without_dummyNA <- train_without_dummyNA[train_without_dummyNA$year_built >="13000",] # deletes 4 obs.
```

Now we move to the imputation part.
...
# using "pmm" method from MICE package, as it needs quite a lot computation we set m=1 and maxit=1
  train_imputed_pmm <- complete(mice(train_imputed, m=2, maxit = 5, meth = "pmm", seed = 50), 1) 
...
train_imputed_pmm$price <- as.integer(train_without_dummyNA$price) # price is a factor variable otherwise
train_imputed_pmm$zipcode <- as.factor(train_without_dummyNA$zipcode) # zipcode should be factor - could even get more sophisticated (e.g. cantone...) with this one if time allows - the tax rate substantially differs across municipatlities in switzerland - that might well affect the prices of aparments

train_lin_subset <- train_imputed_pmm %>% select(-c("year", "zipcode"))
```
...
Our validation set error is pretty high, and deleting outlier observations does not change this. We should try out different approaches. 
