...
Although with multiple imputed data sets we would reach better accuracy, it is only advisable to use multiple imputed data sets assuming we have a linera 
...
# using "pmm" method from MICE package, as it needs quite a lot computation we set m=1 and maxit=1
  train_imputed_pmm <- mice(train_without_dummyNA, m=1, maxit = 20, meth = "pmm", seed = 50)
write.csv(train_imputed_pmm, file = "./train_imputed_pmm.csv", row.names = FALSE)
 
# using "rf" method from MICE package
  train_imputed_rf <- mice(train_without_dummyNA, m=1, maxit = 20, meth = "rf", seed = 50)
...
# So I try to create zipcode dummies manually and when doing the prediction part, will not encounter new levels for zipcode. But I just create with the first two digits of zipcode otherwise it will be tooooo long haha:))
...
# I will try adding some interaction term or model selection later on :D
```
