...
train_without_dummyNA
```

Let us check for some specific numeric variables, if there are some significant outliers in the dataset.

```{r}
plot(train_without_dummyNA$area) # not real outliers,but when check manually some observations with area=1, which does not make sense...but we have them in test data
...
train_without_dummyNA$price <- scale(train_without_dummyNA$price)[,1] # price is a factor variable otherwise
...
lines(density(train_imputed_cart$area), col = "yellow")
...
lines(density(train_imputed_cart$floors), col = "yellow")
...
lines(density(train_imputed_cart$rooms), col = "yellow")
...
lines(density(train_imputed_cart$year_built), col = "yellow")

train_forcorr <- cor(train_imputed_pmm %>% keep(is.numeric) %>% dplyr::select(-starts_with("zipcode")))
corrplot(train_forcorr, method = "circle")

train_forcorr <- cor(train_imputed_norm %>% keep(is.numeric) %>% dplyr::select(-starts_with("zipcode")))
corrplot(train_forcorr, method = "circle")

train_forcorr <- cor(train_imputed_rf %>% keep(is.numeric) %>% dplyr::select(-starts_with("zipcode")))
corrplot(train_forcorr, method = "circle")
...
plot(train_imputed_cart$floors, train_imputed_cart$price) # hard to tell
...
for (i in 1:5) {
  ...
  cv.error.5[i] <- cv.glm(train.data, fit_area, K = 5)$delta[1]
}
...
for (i in 1:5) {
  fit_floors <- glm(price ~ . + area:rooms + floors:elevator + area:floors - floors  + poly(floors,df = i)
             + area:zipcode_11 + area:zipcode_12 + area:zipcode_13 + area:zipcode_14 + area:zipcode_15 
              ...
              + area:zipcode_90 + area:zipcode_91 + area:zipcode_92 + area:zipcode_93 + area:zipcode_94 
              + area:zipcode_95 + area:zipcode_96, data = train.data)
  cv.error.5[i] <- cv.glm(train.data, fit_floors, K = 5)$delta[1]
}
...
plot(cv.error.rooms, xlab = "Degrees of polynomial for rooms", ylab = "Cross-validation error", type = "b", col = "blue") # we set degrees of polynomial for rooms as 5

lm_fit2 <- lm (price ~ . -area - floors - rooms + area:rooms + floors:elevator + area:floors + poly(area, 4) + poly(floors, 3) +poly(rooms, 5)
              + area:zipcode_11 + area:zipcode_12 + area:zipcode_13 + area:zipcode_14 + area:zipcode_15 
              + area:zipcode_16 + area:zipcode_17 + area:zipcode_18 + area:zipcode_19 + area:zipcode_20 
              + area:zipcode_21 + area:zipcode_22 + area:zipcode_23 + area:zipcode_24 + area:zipcode_25 
              + area:zipcode_26 + area:zipcode_27 + area:zipcode_28 + area:zipcode_29 + area:zipcode_30 
...
## Moing to nonlinearity: Smooth Spline and GAM
...
## Moving to Ridge, LASSO and Elastic-net regression
...
# from results, we can see Lasso (alpha = 1) performs best.
...
sqrt(mean((test_y - lasso_predicted)^2))
...
# Random forest
# code from: https://github.com/StatQuest/random_forest_demo/blob/master/random_forest_demo.R

set.seed(1)
rf_fit <- randomForest(price ~ . , data = train.data, mtry = 3, ntree = 25, proximity = TRUE) #Here use default mrty and ntree, but it seems I don't have enough memory for rf, always returns error: cannot allocate vector of size 5.8 Gb

# find optimal value of mtry or ntree, here i input ntree = 25
oob.values <- vector(length = 10)
for (i in 1:10){
  temp.model <- randomForest(price ~ ., data = train.data, mtry = i, ntree = 25)
  oob.values[i] <- temp.model$err.rate[nrow(temp.model$err.rate), 1]
}

rf_testyhat <- predict(rf_fit, newdata = test.data)
sqrt(mean((test.data$price - rf_testyhat)^2))
```
